...
# Função Mestra para Formatação de Moeda Brasileira
...
df_f = df_f[(df_f['Operador'].astype(str).isin(sel_operadores)) & (df_f['Status'].astype(str).isin(sel_status)) & (df_f['Cidade'].astype(str).isin(sel_cidades))]
...
with tab_perdas:
    ...
    if df_p.empty:
        ...
        with cp1:
            ...
            fig_pie.update_traces(textinfo='percent+label', hovertemplate="Motivo: %{label}<br>Valor: R$ %{value:,.2f}")
            st.plotly_chart(fig_pie, use_container_width=True)
        with cp2:
            st.subheader("Perdas por Operador (R$)")
            df_p_op = df_p.groupby('Operador')['Valor Estimado'].sum().reset_index()
            fig_p_bar = px.bar(df_p_op, x='Operador', y='Valor Estimado', color_discrete_sequence=['#FF4B4B'])
            fig_p_bar.update_traces(texttemplate="R$ %{y:,.2f}", textposition='outside')
